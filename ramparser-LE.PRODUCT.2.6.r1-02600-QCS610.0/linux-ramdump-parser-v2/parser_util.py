...
def replace_parser(longopt, desc, shortopt=None, optional=False):
    """
    ...
    def wrapper(cls):
        for p in _parsers:
            if p.longopt == longopt:
                _parsers[_parsers.index(p)] = ParserConfig(cls, longopt, desc, shortopt, optional)
        return cls
